...
function DashboardContent() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const isNew = typeof window !== 'undefined' && sessionStorage.getItem('playful_new') === '1'
      if (isNew) {
        sessionStorage.removeItem('playful_new')
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#06040f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ width: 46, height: 46, borderRadius: '50%', border: '2px solid rgba(255,122,0,0.15)', borderTopColor: '#ff7a00', animation: 'spin 0.8s linear infinite' }} />
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#06040f', color: '#f0eaff', fontFamily: "'Space Mono', monospace", overflowX: 'hidden', WebkitFontSmoothing: 'antialiased' }}>
      ...
      {/* Background layers */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, background: 'radial-gradient(ellipse 55% 45% at 8% 12%, rgba(255,122,0,0.15) 0%,transparent 60%), radial-gradient(ellipse 45% 40% at 92% 82%, rgba(255,222,102,0.08) 0%,transparent 55%), radial-gradient(ellipse 35% 30% at 50% 50%, rgba(255,122,0,0.03) 0%,transparent 70%)' }} />
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, backgroundImage: 'linear-gradient(rgba(255,122,0,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(255,122,0,0.025) 1px,transparent 1px)', backgroundSize: '64px 64px', maskImage: 'radial-gradient(ellipse 80% 80% at 50% 50%,black 0%,transparent 85%)' }} />
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
        ...
        <nav style={{
          ...
          <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 11, textDecoration: 'none' }}>
            <img src="/logo.png" alt="Playful" style={{
              ...
              boxShadow: '0 0 14px rgba(255,122,0,0.2)',
            }} />
            <span style={{
          ...
          <div ref={profileMenuRef} style={{ position: 'relative' }}>
            <button onClick={() => setShowProfileMenu(!showProfileMenu)} style={{
              ...
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#ff7a00" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="3.5"/><path d="M5 19c1.8-3.5 5-5 7-5s5.2 1.5 7 5"/></svg>
            </button>
            ...
            {showProfileMenu && (
              <div style={{
                ...
                border: '1px solid rgba(255,122,0,0.18)', borderRadius: 20, padding: 20,
                ...
                <div style={{ height: 1, background: 'rgba(255,122,0,0.18)', margin: '12px 0' }} />
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 10 }}>
                  ...
                  <span style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.76rem', letterSpacing: '0.06em', color: '#ffe066' }}>{plan}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginTop: 8 }}>
                  ...
                  <span style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.76rem', letterSpacing: '0.06em', color: '#ffe066' }}>{buildsLeft}</span>
                </div>
                <div style={{ height: 1, background: 'rgba(255,122,0,0.18)', margin: '12px 0' }} />
                ...
                <div style={{ marginTop: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 }}>
                    ...
                    <span style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.76rem', letterSpacing: '0.06em', color: '#ffe066' }}>{credits}/{maxCredits}</span>
                  </div>
                  <div style={{ height: 8, borderRadius: 999, background: 'rgba(255,122,0,0.1)', border: '1px solid rgba(255,122,0,0.14)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 999, background: 'linear-gradient(90deg,#ff7a00,#ffe066)', boxShadow: '0 0 10px rgba(255,122,0,0.4)', transition: 'width 0.7s cubic-bezier(0.4,0,0.2,1)', width: `${creditsPct}%` }} />
                  </div>
                </div>
        ...
        <div style={{ width: '100%', maxWidth: 860, margin: '0 auto', padding: '0 clamp(16px,4vw,40px)' }}>
          <div style={{ padding: 'clamp(60px,10vh,100px) 0 clamp(40px,6vh,64px)', textAlign: 'center' }}>
            <div style={{
              ...
              color: '#ffe066', fontSize: '0.67rem', letterSpacing: '0.1em', textTransform: 'uppercase',
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', animation: 'fadeUp 0.6s ease 0.25s both' }}>
              ...
              <button onClick={() => fetchUserData(user)} style={{
                padding: '13px 22px', border: '1px solid rgba(255,122,0,0.18)', borderRadius: 12, cursor: 'pointer',
                fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.7rem', letterSpacing: '0.08em',
                background: 'rgba(255,122,0,0.07)', color: '#ffe066', transition: '0.22s ease',
              }}>Refresh</button>
            </div>
          </div>
        </div>

        <div style={{ maxWidth: 860, margin: '0 auto', height: 1, background: 'linear-gradient(90deg,transparent,rgba(255,122,0,0.18),transparent)' }} />
        ...
        <div style={{ width: '100%', maxWidth: 860, margin: '0 auto', padding: '0 clamp(16px,4vw,40px)' }}>
          <div style={{ padding: 'clamp(40px,6vh,60px) 0' }}>
            <SectionLabel>Welcome reward</SectionLabel>
            <button onClick={() => setShowScratchCard(true)} style={{
              width: '100%', padding: '28px 24px', borderRadius: 18,
              background: 'rgba(14,8,30,0.78)', border: '1px solid rgba(255,122,0,0.18)',
              backdropFilter: 'blur(20px)', cursor: 'pointer', textAlign: 'left',
              position: 'relative', overflow: 'hidden', transition: '0.22s ease',
            }}>
              <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 55% at 50% 0%,rgba(255,122,0,0.07),transparent 70%)', pointerEvents: 'none' }} />
              <div style={{ position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.82rem', letterSpacing: '0.1em', color: '#fff', marginBottom: 6 }}>🎁 Mystery Scratch Card</div>
                  <div style={{ color: 'rgba(240,234,255,0.52)', fontSize: '0.76rem' }}>Scratch to reveal your welcome reward</div>
                </div>
                <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 900, fontSize: '1.4rem', color: '#ffe066' }}>?</div>
              </div>
            </button>
          </div>
        </div>
      ...
      {showScratchCard && (
        <ScratchCardModal
          ...
          copied={copied}
          onCopy={() => { navigator.clipboard.writeText('WAITMAN1'); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
        />
      )}
    </div>
  )
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{
      ...
      color: '#ff7a00', fontWeight: 700, marginBottom: 20,
    }}>
      {children}
      <span style={{ flex: 1, height: 1, background: 'rgba(255,122,0,0.18)' }} />
    </div>
  )
}
...
function UserScroll({ users, totalUsers }: { users: any[]; totalUsers: number }) {
  ...
  const renderChip = (u: any, idx: number) => (
    <div key={`${u.id || idx}`} style={{
      ...
      borderBottom: '1px solid rgba(255,122,0,0.07)', fontSize: '0.76rem', color: 'rgba(200,185,255,0.75)',
    }}>
      <div style={{
    ...
    <div style={{ width: '100%', maxWidth: 860, margin: '0 auto', padding: '0 clamp(16px,4vw,40px)' }}>
      <div style={{ padding: '0 0 clamp(40px,6vh,60px)' }}>
        ...
        <div style={{
          ...
          border: '1px solid rgba(255,122,0,0.18)', backdropFilter: 'blur(20px)',
          overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', maxHeight: 260,
        }}>
          {[colA, colB].map((col, ci) => (
            <div key={ci} style={{ overflow: 'hidden', position: 'relative', borderRight: ci === 0 ? '1px solid rgba(255,122,0,0.18)' : undefined }}>
          ...
          <div style={{
            ...
            borderTop: '1px solid rgba(255,122,0,0.18)', textAlign: 'center',
            fontSize: '0.66rem', color: 'rgba(240,234,255,0.26)', letterSpacing: '0.05em',
            background: 'rgba(255,122,0,0.03)',
          }}>
            <span style={{ color: '#ffe066', fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.7rem' }}>{totalUsers}</span> creators in the queue
          </div>
        </div>
      </div>
    </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, alignItems: 'start' }}>
          {plans.map((p) => (
            <div key={p.name} style={{
              ...
              border: `1px solid ${p.featured ? 'rgba(255,122,0,0.4)' : 'rgba(255,122,0,0.18)'}`,
              display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden',
              boxShadow: p.featured ? '0 0 0 1px rgba(255,122,0,0.2),0 18px 50px rgba(255,122,0,0.12)' : 'none',
            }}>
              ...
              <div style={{ fontSize: '0.6rem', textTransform: 'uppercase', letterSpacing: '0.1em', color: p.featured ? '#ffe066' : 'rgba(240,234,255,0.52)', marginBottom: 12 }}>{p.tag}</div>
              ...
              <div style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.76rem', letterSpacing: '0.12em', color: '#ffe066', marginBottom: 7 }}>{p.name}</div>
              <div style={{ color: 'rgba(240,234,255,0.52)', fontSize: '0.76rem', lineHeight: 1.65, marginBottom: 16, minHeight: 42 }}>{p.desc}</div>
              <div style={{ height: 1, background: 'rgba(255,122,0,0.18)', marginBottom: 14 }} />
              <div style={{ display: 'flex', flexDirection: 'column', gap: 9, flex: 1, marginBottom: 16 }}>
                {p.feats.map((f) => (
                  <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: '0.76rem', color: 'rgba(200,185,255,0.8)', lineHeight: 1.45 }}>
                    <span style={{ width: 16, height: 16, minWidth: 16, marginTop: 1, borderRadius: 5, background: p.featured ? 'rgba(255,122,0,0.22)' : 'rgba(255,122,0,0.12)', border: `1px solid ${p.featured ? 'rgba(255,122,0,0.3)' : 'rgba(255,122,0,0.18)'}`, display: 'grid', placeItems: 'center', fontSize: '0.58rem' }}>✓</span>
                    {f}
                  </div>
                ))}
                {p.dimFeats.map((f) => (
                  <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, fontSize: '0.76rem', color: 'rgba(200,185,255,0.8)', lineHeight: 1.45, opacity: 0.36, textDecoration: 'line-through' }}>
                    <span style={{ width: 16, height: 16, minWidth: 16, marginTop: 1, borderRadius: 5, background: 'rgba(255,122,0,0.12)', border: '1px solid rgba(255,122,0,0.18)', display: 'grid', placeItems: 'center', fontSize: '0.58rem' }}>✓</span>
                    {f}
                  </div>
                ))}
              </div>
              <div style={{ fontSize: '0.66rem', color: 'rgba(240,234,255,0.26)', fontStyle: 'italic', padding: '9px 0', borderTop: '1px solid rgba(255,122,0,0.18)', marginBottom: 13 }}>{p.vibe}</div>
              <button disabled={p.btnDisabled} style={{
                border: p.btnDisabled ? '1px solid rgba(255,122,0,0.18)' : 'none',
                ...
                background: p.btnDisabled ? 'rgba(255,122,0,0.06)' : p.featured ? 'linear-gradient(135deg,#ff7a00,#ff4d00)' : 'linear-gradient(135deg,#ffe066,#ff7a00)',
...
/* ── SCRATCH CARD MODAL ── */
function ScratchCardModal({ onClose, revealed, onReveal, copied, onCopy }: {
  onClose: () => void; revealed: boolean; onReveal: () => void; copied: boolean; onCopy: () => void
}) {
  ...
  useEffect(() => {
    ...
    function onMove(e: MouseEvent | TouchEvent) {
      ...
      const now = Date.now()
      if (now - lastCheck.current > 80) {
        lastCheck.current = now
        if (pctScratched() >= 45 && !revealedRef.current) {
          onReveal()
          setTimeout(eraseAll, 220)
        }
      }
    }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 800, background: 'rgba(0,0,0,0.80)', backdropFilter: 'blur(18px)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'fadeIn 0.2s ease' }}>
      ...
      <div style={{ position: 'relative', width: 'min(86vw,380px)', aspectRatio: '4/3' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: 'linear-gradient(90deg,transparent,rgba(255,122,0,0.55),transparent)' }} />
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', borderRadius: 'inherit', background: 'radial-gradient(ellipse 70% 50% at 50% -4%,rgba(255,122,0,0.13),transparent 58%)' }} />

          <div style={{ width: 46, height: 46, borderRadius: 12, overflow: 'hidden', background: '#06040f', border: '1.5px solid rgba(255,122,0,0.38)', boxShadow: '0 0 14px rgba(255,122,0,0.32)', flexShrink: 0, position: 'relative', zIndex: 1, animation: 'scBob 3s ease-in-out infinite' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1, background: '#000', border: '1px solid rgba(255,122,0,0.35)', borderRadius: 9, padding: '8px 44px 8px 13px', display: 'flex', alignItems: 'center', minWidth: 170, boxShadow: '0 0 16px rgba(255,122,0,0.12)' }}>
            <span style={{ fontFamily: "'Orbitron',sans-serif", fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.2em', color: '#fff', flex: 1 }}>WAITMAN1</span>
            <button onClick={onCopy} style={{ position: 'absolute', right: 9, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: copied ? '#86efac' : 'rgba(255,122,0,0.5)', padding: 2 }}>
          ...
          <div style={{ fontSize: '0.5rem', color: 'rgba(255,255,255,0.22)', letterSpacing: '0.09em', textTransform: 'uppercase', position: 'relative', zIndex: 1, textAlign: 'center', lineHeight: 1.7 }}>🎮 Valid <b style={{ color: 'rgba(255,122,0,0.5)', fontWeight: 400 }}>30 days</b> · One use · Apply at checkout</div>
        </div>
        ...
        {!revealed && (
          <div style={{ position: 'absolute', bottom: -32, left: '50%', transform: 'translateX(-50%)', fontSize: '0.54rem', color: 'rgba(255,122,0,0.5)', letterSpacing: '0.17em', textTransform: 'uppercase', whiteSpace: 'nowrap', animation: 'scHint 1.6s ease-in-out infinite', pointerEvents: 'none', zIndex: 11 }}>
