...
/* ── Arc Carousel + Footer — single 100dvh snap section ── */
    ...
    <section ref={sectionRef} style={{
      ...
      <div style={{ flex: '0 0 auto', paddingTop: 'clamp(20px,4vh,50px)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 'clamp(16px,3vh,40px)' }}>
          ...
          <span style={{ fontSize: '.65rem', color: 'rgba(0,200,255,.8)', textTransform: 'uppercase' as const, letterSpacing: '.3em', fontWeight: 700 }}>
        ...
        <div ref={stageRef} style={{ position: 'relative', width: '100%', height: 'clamp(200px,30vh,280px)' }}>
          {ARC_CARDS.map((c, i) => (
            <div key={i} className="arc-card" style={{
              ...
              <div style={{ fontFamily: 'var(--font-orbitron,Orbitron,sans-serif)', fontSize: '.9rem', fontWeight: 700, marginBottom: 6 }}>{c.n}</div>
              <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.5)', lineHeight: 1.4 }}>{c.d}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <footer style={{
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 340 }}>
          ...
          <p style={{ fontSize: '.78rem', color: 'rgba(255,255,255,.45)', lineHeight: 1.5, marginBottom: 14 }}>
            Turn your words into worlds. Type a prompt, get a playable game in seconds.
          </p>
          <h4 style={{ fontFamily: 'var(--font-orbitron,Orbitron,sans-serif)', color: '#fff', fontSize: '.75rem', marginBottom: 10, letterSpacing: '.05em', textTransform: 'uppercase' as const }}>
            Join our society
          </h4>
        ...
        <div style={{ display: 'flex', gap: 'clamp(24px,4vw,60px)', flexWrap: 'wrap' as const }}>
          {[
            ...
            <div key={h} style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
              <h4 style={{ fontFamily: 'var(--font-orbitron,Orbitron,sans-serif)', color: '#fff', fontSize: '.78rem', marginBottom: 4, letterSpacing: '.05em' }}>{h}</h4>
              {links.map(l => (
                <a key={l} href="#" className="foot-link" style={{ color: 'rgba(255,255,255,0.45)', textDecoration: 'none', fontSize: '.78rem', transition: 'color 0.2s' }}>{l}</a>
              ))}
            </div>
          ))}
        </div>
      ...
      {/* Copyright */}
      <div style={{ maxWidth: 1200, margin: 'clamp(12px,1.5vh,20px) auto 0', width: '100%', textAlign: 'center' as const, fontSize: '.78rem', color: 'rgba(255,255,255,0.45)' }}>
        © Playful, Developed by{' '}
        <span style={{ fontFamily: "'Cinzel Decorative', serif", fontWeight: 700, color: '#fff', letterSpacing: '0.1em' }}>SURYA</span> ✨
      </div>
    </footer>
  )
      }
